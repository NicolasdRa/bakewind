...
export const inventoryItems = pgTable(
  'inventory_items',
  {
    ...
    // Tenant reference - which bakery owns this inventory
  ...
  (table) => ({
    ...
    idxInventoryTenant: index('idx_inventory_tenant').on(table.tenantId),
    // Composite indexes for common queries
    idxInventoryTenantCategory: index('idx_inventory_tenant_category').on(
      table.tenantId,
      table.category,
    ),
    idxInventoryTenantLowStock: index('idx_inventory_tenant_low_stock').on(
      table.tenantId,
      table.currentStock,
      table.minimumStock,
    ),
