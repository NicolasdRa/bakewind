...
CREATE INDEX "idx_session_active_user" ON "user_sessions" USING btree ("user_id","is_revoked","expires_at") WHERE "user_sessions"."is_revoked" = false AND "user_sessions"."expires_at" > NOW();--> statement-breakpoint
CREATE INDEX "idx_session_cleanup" ON "user_sessions" USING btree ("expires_at","is_revoked") WHERE "user_sessions"."expires_at" < NOW() OR "user_sessions"."is_revoked" = true;--> statement-breakpoint
